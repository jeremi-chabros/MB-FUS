...
data.long<-data.frame(
  cfDNA_Ratio=unlist(data.used[,c("c1","c2","c3",
                                  "C4","c5","c6")]),
...
data.long<-data.long[complete.cases(data.long$cfDNA_Ratio),]
# View(data.long)
#length(unique(data.long$Id))
data.long$cfDNA_Ratio <- sapply(data.long$cfDNA_Ratio,as.numeric)

#group1
#p <- ggplot(data = data.long, aes(x = Cycle, y = cfDNA_Ratio, 
...
#group2
#p <- ggplot(data = data.long, aes(x = Cycle, y = cfDNA_Ratio, group = Id, color=Group1))
...
#statistical test
#data.long.smaller<-data.long
#data.long.smaller$Cycle<-as.numeric(data.long.smaller$Cycle)
#fit<-lme(cfDNA_Ratio ~ Cycle*Group1,
#         random=~1|Id,
#         data=data.long.smaller)
#result<-summary(fit)
#result

#result<-geeglm(cfDNA_Ratio~ Cycle*Group1, data=data.long.smaller,id= Id, corstr="exchangeable") 
#summary(result)

#data.long.smaller<-data.long[ data.long$Group1=="Early Progressors",]
#data.long.smaller$Cycle<-as.numeric(data.long.smaller$Cycle)
#fit<-lme(cfDNA_Ratio ~ Cycle,
#         random=~1|Id,
#         data=data.long.smaller)
#result<-summary(fit)
#result#

#result<-geeglm(cfDNA_Ratio~ Cycle, data=data.long.smaller,id= Id, corstr="exchangeable") 
#summary(result)
...
# data.used.filter<-data.used[!is.na(data.used$c4),]
# data.peak.last<-data.frame(
#   cfDNA_Ratio=c(as.vector(apply(data.used.filter[,c("c1","c2","c3",
#                                  "c4")],1,function (x)
#                                  {
#                                    max(x,na.rm = T)
#                                  })), unlist(data.used.filter[,"c4"])),
...
# )
# data.peak.last<-data.peak.last[! is.infinite(data.peak.last$cfDNA_Ratio),]
# head(data.peak.last)
# 
# data.peak.last<-data.peak.last[complete.cases(data.peak.last$cfDNA_Ratio),]
# 
# data.peak.last$Group2<-relevel(as.factor(data.peak.last$Group2),ref="Peak cycle")
# 
# fit<-lme(cfDNA_Ratio ~Group1*Group2,
#          random=~1|Id,
#          data=data.peak.last)
# result<-summary(fit)
# result
...
#   "2",     "3", "ns"
# )
# p <- ggplot(data.peak.last, aes(x=Group2, y=cfDNA_Ratio,col=Group1,group=Id)) + 
#   #geom_violin(draw_quantiles = c(.25, .50, .75)) +
#   geom_line(size=0.8,aes(x=Group2,y=cfDNA_Ratio, group = Id)) +
#   # geom_line(aes(x=date,y=value, color = adjuster, group = 1)) +
#   geom_point(size=1.3,aes(x=Group2,y=cfDNA_Ratio, group = Id)) +
#   ylab("cfDNA Ratio") +
#   xlab("Cycle") +
#   scale_color_manual(values=c( "#E69F00", "skyblue")) +
...
# data.peak.last<-data.frame(
#   cfDNA_Ratio=c(unlist(data.used.filter[,"c4"])/as.vector(apply(data.used.filter[,c("c1","c2","c3",
#                                              "c4")],1,function (x)
#                                              {
#                                                max(x,na.rm = T)
#                                              }))),
...
#   "2",     "3", "ns"
# )
# p <- ggplot(data.peak.last, aes(x=Group1, y=cfDNA_Ratio,fill=Group1)) + 
#   # geom_violin(draw_quantiles = c(.25, .50, .75)) +
#   geom_violin() +
...
#   #   label = "p.adj"
#   # ) +
#   ylab("Normalized Peak cfDNA Ratio") +
#   xlab("Group") +
...
# 
# t.test(
#   data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Early Progressors"],
#   data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Long-term Survivors"],
#   var.equal=F
# )
# 
...
data.used.filter$last_obs<-as.numeric(apply(data.used.filter[,c("c1","c2","c3",
                                                     "C4","c5","c6")], 1, 
                                 function (x) 
                                 {
...
data.peak.last<-data.frame(
  cfDNA_Ratio=as.numeric(c(as.vector(apply(data.used.filter[,c("c1","c2","c3",
                                                    "C4","c5","c6")],1,function (x)
                                                       {
                                                         max(x,na.rm = T)
...
data.peak.last<-data.frame(
  cfDNA_Ratio=c(unlist(data.used.filter[,"last_obs"])/as.numeric(as.vector(apply(data.used.filter[,c("c1","c2","c3",
                                                                                             "C4","c5","c6")],1,function (x)
                                                                                             {
                                                                                               max(x,na.rm = T)
                                                                                             })))),
...
# t.test(data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Long-term Survivor"], mu=1)
#t.test(
#  data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Early Progressors"],
#  data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Long-term Survivors"],
#  var.equal=F
#)
# wilcox.test(data.peak.last$cfDNA_Ratio[data.peak.last$Group1=="Long-term Survivor"], mu=1)
# wilcox.test(data.peak.last$cfDNA_Ratio ~ Group1, data = data.peak.last, alternative = "two.sided")
...
active_tumor<-c(1.11,	1.85,	(2.58+3.59)/2,	2.2825)
data_three_groups<-data.frame(Outcome=as.numeric(c(active_tumor,data.used.filter$last_obs)),
      Group=c(rep("Active Tumor",4),data.used.filter$Group1))
...
stat.test <- tibble::tribble(
  ...
  #"1",     "2", "ns",
  #"1",     "3", "ns",
  "Active Tumor",     "Early Progressors", "ns",
  "Active Tumor",     "Long-term Survivors", "ns",
  "Early Progressors",     "Long-term Survivors", "***"
)
t.test(data_three_groups$Outcome[data_three_groups$Group=="Long-term Survivors"],data_three_groups$Outcome[data_three_groups$Group=="Early Progressors"])
p <- ggboxplot(data_three_groups,x="Group", y="Outcome",fill="Group",
  ...
  stat_pvalue_manual(
    stat.test,
    size=7,
    y.position = 4.2, step.increase = 0.1,
    label = "p.adj"
  ) +
  # geom_violin() +
  #geom_boxplot(width=1) +  # Adjust width and position as needed
  
  ylab("Last Observed cfDNA Ratio") +
  xlab("Group") +
...
#data.used.filter$C4 <- sapply(data.used.filter$C4,as.numeric)
